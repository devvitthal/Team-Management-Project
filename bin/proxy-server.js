...
try {
 ...
 const match =
  envContent.match(/VITE_API_ENDPOINTS='(.+)'/) ||
  envContent.match(/REACT_APP_API_ENDPOINTS='(.+)'/);
 if (match) {
...
const server = http.createServer((req, res) => {
   ...
   JSON.stringify(
    {
     ...
     endpoints: Object.keys(endpoints).map(
      (name) => `http://localhost:${PORT}/api/${name}`,
     ),
    },
    null,
    2,
 ...
 // Forward the remaining sub-path after /api/{service-name} to the Lambda Function URL.
 // e.g. /api/auth-service/auth/login -> Lambda URL + /auth/login
 const remainingPath =
  pathParts.length > 2 ? "/" + pathParts.slice(2).join("/") : "/";
 const baseEndpointUrl = (endpoints[endpointName] || "").replace(/\/$/, "");
 const targetUrl = baseEndpointUrl + remainingPath + (parsedUrl.search || "");
